...
if (isset($_POST['xAccion'])) {
     ...
     if ($_POST['xAccion'] == 'grabar') {
          ...
          $clasf3->setStatus(2);
          $clasf3->setNumeroGuia($_POST['guia']);
           $clasf3->setDescripcionGuia($_POST['paqueteria']);
          $count = $clasf3->grabar();
            if ($count != 0) {
...
<html lang="es">
    ...
    <div id="wrapper">
         ...
         <!-- Navigation -->
           ...
           <form role="form" name="pedidosEntrantes" id="pedidosEntrantes" action="<?php echo($_SERVER['PHP_SELF']); ?>" method="POST">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">Nuevos Pedidos</h1>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
    ...
    <tbody>
        <?php
        $sql = "SELECT * from pedidos where status in (1)  order by fecha ";
        ...
        foreach ($rst as $row) {
            ...
            <div id="envio<?PHP echo($row['cve_pedido']); ?>" class="modalmask">
                <div class="modalbox resize">
                    ...
                    <h3 id="TituloModal">Datos del envío</h3>
                        <div class="form-group">
                        <label for="txtPaqueteria">Paquetería:</label>
                        <input type="text" class="form-control" id="txtPaqueteria<?PHP echo $row['cve_pedido'];?>" name="txtPaqueteria" 
                               placeholder="Estafeta " value="">
                    </div>
                        <div class="form-group">
                        <label for="txtGuia">Número de Guía:</label>
                        <input type="text" class="form-control" id="txtGuia<?PHP echo $row['cve_pedido'];?>" name="txtGuia" 
                               placeholder="Número de la guía " value="">
                    </div>
                     <button type="button" class="btn btn-default" id="btnGrabar" name="btnGrabar" onclick="grabar(<?PHP echo $row['cve_pedido'];?>,'#txtPaqueteria<?PHP echo $row['cve_pedido'];?>','#txtGuia<?PHP echo $row['cve_pedido'];?>');">Enviar</button>
                </div>
